...
<section class="relative min-h-[90vh] flex items-center justify-center overflow-hidden"
         x-data="{
             slides: {{ isset($banners) && $banners->count() ? $banners->count() : 1 }},
    ...
    <div class="absolute inset-0">
        @forelse ($banners ?? [] as $i => $banner)
            <div x-show="current === {{ $i }}"
                ...
                <img src="{{ asset('storage/' . $banner->gambar) }}" alt="{{ $banner->judul ?? 'Hero Banner' }}" class="w-full h-full object-cover">
    ...
    @if (isset($banners) && $banners->count() > 1)
        ...
        <div class="absolute bottom-20 sm:bottom-24 left-1/2 -translate-x-1/2 z-20 flex gap-2">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center pt-32 sm:pt-28 pb-24 sm:pb-28" x-data="{ counted: false }" x-intersect="counted = true">
        ...
        <h1 class="text-4xl sm:text-6xl font-bold text-white leading-tight tracking-tight" style="font-family: 'Plus Jakarta Sans', sans-serif;">
            @if (isset($banners) && $banners->isNotEmpty())
                <span x-text="banners[current]?.judul || 'Tebing Tinggi Okura'"></span>
            @else
                Tebing Tinggi Okura
            @endif
        </h1>
        ...
        <p class="mt-4 text-lg sm:text-xl text-slate-200 max-w-2xl mx-auto">
            @if (isset($banners) && $banners->isNotEmpty())
                <span x-text="banners[current]?.subjudul || 'Menyajikan pelayanan, informasi, dan potensi wisata & UMKM warga secara cepat, transparan, dan modern.'"></span>
            @else
        ...
        <div class="mt-12 mb-2 grid grid-cols-3 gap-4 max-w-lg mx-auto">
            <div class="text-center" x-data="{ val: 0, target: {{ $jumlahPenduduk ?? 0 }} }" x-effect="if (counted && val === 0 && target > 0) { let step = Math.max(1, Math.ceil(target / 80)); let t = setInterval(() => { val += step; if (val >= target) { val = target; clearInterval(t); } }, 15) }">
            ...
            <div class="text-center" x-data="{ val: 0, target: {{ $jumlahWisata ?? 0 }} }" x-effect="if (counted && val === 0 && target > 0) { let t = setInterval(() => { val += 1; if (val >= target) { val = target; clearInterval(t); } }, 80) }">
            ...
            <div class="text-center" x-data="{ val: 0, target: {{ $jumlahUmkm ?? 0 }} }" x-effect="if (counted && val === 0 && target > 0) { let t = setInterval(() => { val += 1; if (val >= target) { val = target; clearInterval(t); } }, 40) }">
                <p class="text-2xl sm:text-3xl font-bold text-amber-300" x-text="val"></p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4">
        ...
        <div class="md:col-span-2 md:row-span-2 bg-white rounded-2xl shadow-md p-6 border border-slate-100 flex flex-col justify-between">
            <div>
                ...
                <div class="w-full h-64 md:h-80 rounded-xl overflow-hidden border border-slate-100 bg-slate-50">
                    <iframe
                        width="100%"
                        height="100%"
                        style="border:0;"
                        loading="lazy"
                        allowfullscreen
                        src="https://maps.google.com/maps?q=0.571220255707136,101.53890031010887&z=16&output=embed">
                    </iframe>
                </div>
            </div>
            <p class="text-xs text-slate-500 mt-3">Kelurahan Tebing Tinggi Okura, Kec. Rumbai Timur, Pekanbaru.</p>
        </div>

        {{-- Visi Misi --}}
        <div class="md:col-span-2 bg-[#0B1F3A] rounded-2xl shadow-md p-6 text-white flex flex-col justify-between">
            <div>
        ...
        {{-- Chart Anggaran --}}
        <div class="bg-amber-50 rounded-2xl shadow-md p-6 border border-amber-100 flex flex-col justify-between">
            <div>
                <h3 class="font-semibold text-amber-800 mb-2">📊 Transparansi Anggaran {{ now()->year }}</h3>
                @if (isset($anggaranTahunIni) && $anggaranTahunIni->count())
                    <canvas id="chartAnggaranHome" height="120"></canvas>
                @else
                    <p class="text-xs text-amber-700 mt-2">Data anggaran belum tersedia untuk tahun ini.</p>
                @endif
            </div>
        </div>
    </div>
...
{{-- ================= FLOATING WHATSAPP DINAMIS TERPERBAIKI ================= --}}
@php
    $nomorWa = data_get($settings, 'whatsapp')
            ?? data_get($settings, 'no_wa')
            ?? data_get($settings, 'nomor_hp')
            ?? data_get($settings, 'telepon')
            ?? '085923287345';

    $pesanWa = data_get($settings, 'pesan_wa')
            ?? data_get($settings, 'pesan_otomatis')
            ?? 'Halo Admin Kelurahan Tebing Tinggi Okura, saya ingin bertanya tentang layanan publik.';
...
<a href="https://api.whatsapp.com/send?phone={{ $nomorWaClean }}&text={{ rawurlencode($pesanWa) }}"
   target="_blank"
