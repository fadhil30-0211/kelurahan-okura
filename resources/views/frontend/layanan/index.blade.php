    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-10">
            @foreach ($jenisSurat as $key => $surat)
                <div class="bg-white rounded-2xl shadow-md border border-slate-100 p-6 flex flex-col justify-between">
                    <div>
                    ...
                    <div class="flex gap-2 mt-auto">
                        ...
                        @if (isset($surat['template']) && file_exists(public_path($surat['template'])))
                            <a href="{{ asset($surat['template']) }}" download
                               class="px-3.5 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 transition flex items-center justify-center" title="Download Template">
