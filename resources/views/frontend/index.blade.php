...
@php
    // Normalisasi $banners & ambil field yang dibutuhkan saja untuk Alpine.js
    $bannersData = isset($banners) && is_iterable($banners) ? collect($banners)->map(function($b) {
        $item = (object) $b;
        return [
            'judul' => $item->judul ?? '',
            'subjudul' => $item->subjudul ?? '',
            'gambar' => $item->gambar ?? '',
            'tombol_teks' => $item->tombol_teks ?? null,
            'tombol_link' => $item->tombol_link ?? null,
        ];
    }) : collect();
    ...
    // Processing Google Maps Embed URL
...
<section class="relative min-h-[90vh] flex items-center justify-center overflow-hidden"
    ...
    @if ($bannerCount > 1)
        <button @click="prev()"
                ...
                class="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-white/30 via-white/10 to-white/5 backdrop-blur-md border border-white/40 hover:border-white/70 shadow-lg shadow-black/25 flex items-center justify-center text-white transition-all duration-300 hover:scale-110 active:scale-95 focus:outline-none group">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 drop-shadow group-hover:-translate-x-0.5 transition-transform shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button @click="next()"
                ...
                class="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-white/30 via-white/10 to-white/5 backdrop-blur-md border border-white/40 hover:border-white/70 shadow-lg shadow-black/25 flex items-center justify-center text-white transition-all duration-300 hover:scale-110 active:scale-95 focus:outline-none group">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 drop-shadow group-hover:translate-x-0.5 transition-transform shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-6 sm:px-12 text-center pt-32 sm:pt-28 pb-24 sm:pb-28">
        ...
        <h1 class="text-3xl sm:text-6xl font-bold text-white leading-tight tracking-tight" style="font-family: 'Plus Jakarta Sans', sans-serif;">
            @if ($bannerCount > 0)
        ...
        <p class="mt-4 text-base sm:text-xl text-slate-200 max-w-2xl mx-auto leading-relaxed">
            @if ($bannerCount > 0)
        ...
        <div id="lacak" class="mt-8 max-w-xl mx-auto" x-data="{ tab: window.location.hash === '#lacak' ? 'lacak' : 'cari' }">
            ...
            <form x-show="tab === 'cari'" action="{{ route('search') }}" method="GET">
                <div class="flex items-center bg-white/15 backdrop-blur-md rounded-2xl shadow-xl p-2 border border-white/30 transition-all duration-300 focus-within:bg-white/25 focus-within:border-white/50 focus-within:ring-4 focus-within:ring-white/10">
                    ...
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari: Syarat SKTM, Wisata Sungai Siak, UMKM..." autocomplete="off"
            ...
            <form x-show="tab === 'lacak'" x-cloak action="{{ route('tracking.universal') }}" method="POST">
                ...
                <div class="flex items-center bg-white/15 backdrop-blur-md rounded-2xl shadow-xl p-2 border border-white/30 transition-all duration-300 focus-within:bg-white/25 focus-within:border-white/50 focus-within:ring-4 focus-within:ring-white/10">
                    ...
                    <input type="text" name="kode_tiket" placeholder="Masukkan kode tiket, contoh: ADU-20260809-001" autocomplete="off"
...
<section class="max-w-6xl mx-auto px-4 sm:px-6 py-16">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4">
        ...
        <div class="md:col-span-2 bg-[#009B3A] rounded-2xl shadow-md p-6 text-white flex flex-col justify-between relative overflow-hidden">
            <div class="relative z-10">
                <h3 class="font-semibold mb-2 flex items-center gap-2 text-amber-300">
                    <svg class="w-5 h-5 text-amber-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                ...
                <div class="space-y-2">
                    @forelse ($pengumumanTerbaru ?? [] as $item)
                        @php $itemObj = (object) $item; @endphp
                        <a href="{{ route('pengumuman.show', $itemObj->id ?? '#') }}" class="block text-xs text-slate-600 hover:text-emerald-600 py-1 border-b border-slate-50 last:border-0 truncate transition-colors">
                            • {{ $itemObj->judul ?? '-' }}
                        </a>
                    @empty
        ...
        {{-- Transparansi Anggaran (Perbaikan SVG Path & layout) --}}
        <div class="bg-white rounded-2xl shadow-md p-6 border border-slate-100 flex flex-col justify-between">
            <div>
                <div class="flex items-center justify-between mb-2">
                    <h3 class="font-semibold text-slate-800 flex items-center gap-2">
                        <svg class="w-6 h-6 text-emerald-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 3.75A1.75 1.75 0 005.25 5.5v13A1.75 1.75 0 007 20.25h10a1.75 1.75 0 001.75-1.75v-13A1.75 1.75 0 0017 3.75H7zM8.5 16v-3m3.5 3V9m3.5 7v-5M8.5 7.5h7"/>
                        </svg>
                ...
                <p class="text-xs text-slate-500 mt-2 leading-relaxed">
                    Akses keterbukaan informasi publik mengenai alokasi dan realisasi APBD Kelurahan.
                </p>
            </div>

            <div class="pt-3">
                <a href="{{ route('transparansi.index') }}" class="inline-flex items-center gap-1 text-emerald-600 text-sm font-medium hover:underline">
                    Rincian <span>→</span>
                </a>
            </div>
        </div>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse ($wisatas ?? [] as $wisata)
                @php $wisataObj = (object) $wisata; @endphp
...
<section class="max-w-6xl mx-auto px-4 sm:px-6 py-16">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
        @forelse ($umkms ?? [] as $umkm)
            @php $umkmObj = (object) $umkm; @endphp
