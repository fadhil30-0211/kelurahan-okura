            ...
            <div class="px-4 py-4 space-y-1">
                ...
                <div class="py-1 px-3">
                    ...
                    <div class="pl-2 border-l-2 border-slate-200 space-y-1">
                        ...
                        @if (Route::has('transparansi.index'))
                            <a href="{{ route('transparansi.index') }}"
                               class="flex items-center gap-2 py-1.5 px-2 rounded text-sm {{ Route::is('transparansi*') ? 'text-emerald-600 font-semibold bg-emerald-50' : 'text-slate-600 hover:text-emerald-600' }}">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                                Transparansi Anggaran
                            </a>
                        @endif
...
{{-- ============ FLASH MESSAGES ============ --}}
<div class="fixed top-20 right-4 z-50 space-y-2 max-w-sm sm:max-w-md w-full px-4 sm:px-0 pointer-events-none">
    @if (session('success'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" x-transition x-cloak
             class="pointer-events-auto bg-emerald-600 text-white px-5 py-3 rounded-xl shadow-lg text-sm font-medium flex items-start justify-between gap-3">
            <div class="flex items-start gap-2">
                <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg>
                <span class="leading-snug">{{ session('success') }}</span>
            </div>
            <button @click="show = false" class="text-white/80 hover:text-white text-lg leading-none">&times;</button>
        </div>
    @endif

    @if (session('info'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition x-cloak
             class="pointer-events-auto bg-sky-600 text-white px-5 py-3 rounded-xl shadow-lg text-sm font-medium flex items-start justify-between gap-3">
            <div class="flex items-start gap-2">
                <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
    ...
    @if (session('error'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition x-cloak
             class="pointer-events-auto bg-rose-600 text-white px-5 py-3 rounded-xl shadow-lg text-sm font-medium flex items-start justify-between gap-3">
            <div class="flex items-start gap-2">
                <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10 pb-10 border-b border-white/10">
            ...
            <div class="md:col-span-2">
                ...
                @if($fbUrl || $igUrl || $ytUrl || $ttUrl)
                    <div class="flex items-center gap-3">
                        @if($fbUrl)
                            <a href="{{ $fbUrl }}" target="_blank" rel="noopener noreferrer" class="w-9 h-9 rounded-lg bg-slate-800 hover:bg-emerald-600 text-white flex items-center justify-center transition" aria-label="Facebook">
                                <i class="fab fa-facebook-f text-sm"></i>
                            </a>
                        @endif
                        @if($igUrl)
                            <a href="{{ $igUrl }}" target="_blank" rel="noopener noreferrer" class="w-9 h-9 rounded-lg bg-slate-800 hover:bg-emerald-600 text-white flex items-center justify-center transition" aria-label="Instagram">
                                <i class="fab fa-instagram text-sm"></i>
                            </a>
                        @endif
                        @if($ytUrl)
                            <a href="{{ $ytUrl }}" target="_blank" rel="noopener noreferrer" class="w-9 h-9 rounded-lg bg-slate-800 hover:bg-emerald-600 text-white flex items-center justify-center transition" aria-label="YouTube">
                                <i class="fab fa-youtube text-sm"></i>
                            </a>
                        @endif
                        @if($ttUrl)
                            <a href="{{ $ttUrl }}" target="_blank" rel="noopener noreferrer" class="w-9 h-9 rounded-lg bg-slate-800 hover:bg-emerald-600 text-white flex items-center justify-center transition" aria-label="TikTok">
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm text-slate-400">
                    ...
                    @if (Route::has('transparansi.index'))
                        <li>
                            <a href="{{ route('transparansi.index') }}" class="hover:text-amber-300 transition flex items-center gap-1.5">
                                <svg class="w-3.5 h-3.5 shrink-0 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="m9 18 6-6-6-6"/></svg>
                                Transparansi Anggaran
                            </a>
                        </li>
