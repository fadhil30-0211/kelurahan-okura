...
<header x-data="{ open: false, scrolled: false }"
    ...
    :class="{
        'bg-white/95 backdrop-blur shadow-sm': scrolled || !{{ $isHome ? 'true' : 'false' }},
        'bg-gradient-to-b from-black/50 to-transparent': !scrolled && {{ $isHome ? 'true' : 'false' }}
    }"
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="flex items-center justify-between h-16 sm:h-20">
            <a href="{{ route('home') }}" class="flex items-center gap-2.5">
                ...
                <span :class="(scrolled || !{{ $isHome ? 'true' : 'false' }}) ? 'text-[#0B1F3A]' : 'text-white'"
            ...
            <a href="{{ route('pengaduan.create') }}"
               class="hidden md:inline-flex items-center px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold transition shadow-sm">
                Lapor Sekarang
            </a>

            <button @click="open = !open" class="md:hidden p-2 rounded-lg"
                    :class="(scrolled || !{{ $isHome ? 'true' : 'false' }}) ? 'text-slate-700' : 'text-white'">
...
{{-- ============ MAIN CONTENT ============ --}}
<main class="{{ $isHome ? '' : 'pt-16 sm:pt-20' }}">
    @yield('content')
</main>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10 pb-10 border-b border-white/10">
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm text-slate-400">
                    ...
                    <li><a href="{{ route('pengumuman.index') }}" class="hover:text-amber-300">Pengumuman</a></li>
                    <li><a href="{{ route('galeri.index') }}" class="hover:text-amber-300">Galeri Kegiatan</a></li>
